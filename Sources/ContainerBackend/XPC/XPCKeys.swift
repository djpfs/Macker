//===----------------------------------------------------------------------===//
// XPCKeys and XPCRoute — the wire protocol of container-apiserver.
//
// These mirror the definitions in apple/container's
// Sources/Services/ContainerAPIService/Client/XPC+.swift. They are the
// (unstable, undocumented) protocol surface we pin to per release.
//===----------------------------------------------------------------------===//
...
/// Routes supported by container-apiserver.
...
extension XPCMessage {
    ...
    public func data(key: XPCKeys) -> Data? { data(key: key.rawValue) }
    public func set(key: XPCKeys, value: Data) { set(key: key.rawValue, value: value) }
    public func string(key: XPCKeys) -> String? { string(key: key.rawValue) }
    public func set(key: XPCKeys, value: String) { set(key: key.rawValue, value: value) }
    public func bool(key: XPCKeys) -> Bool { bool(key: key.rawValue) }
    public func set(key: XPCKeys, value: Bool) { set(key: key.rawValue, value: value) }
    public func uint64(key: XPCKeys) -> UInt64 { uint64(key: key.rawValue) }
    public func set(key: XPCKeys, value: UInt64) { set(key: key.rawValue, value: value) }
    public func int64(key: XPCKeys) -> Int64 { int64(key: key.rawValue) }
    public func set(key: XPCKeys, value: Int64) { set(key: key.rawValue, value: value) }
    public func fileHandle(key: XPCKeys) -> FileHandle? { fileHandle(key: key.rawValue) }
    public func set(key: XPCKeys, value: FileHandle) { set(key: key.rawValue, value: value) }
    public func fileHandles(key: XPCKeys) -> [FileHandle]? { fileHandles(key: key.rawValue) }
    public func set(key: XPCKeys, value: [FileHandle]) { set(key: key.rawValue, value: value) }

    public func decodeJSON<T: Decodable>(_ type: T.Type, key: XPCKeys) throws -> T {
        try decodeJSON(type, key: key.rawValue)
    }

    public func setJSON<T: Encodable>(_ value: T, key: XPCKeys) throws {
        try setJSON(value, key: key.rawValue)
    }
}
